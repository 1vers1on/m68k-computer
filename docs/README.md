...
The pages cover the memory map, bus and address decode, clock and reset, power control, interrupts, system control, debug display, DRAM, firmware ROM, BIOS backup SRAM, MFP, the DMA controller, floppy and IDE storage, VGA, OPL3, MIDI, RTC, PI/T, and expansion slots. Each page records its own revision and design status.
